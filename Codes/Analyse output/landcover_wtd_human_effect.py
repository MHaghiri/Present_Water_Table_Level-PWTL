...
OUTPUT_BASENAME = "landcover_wtd_human_effect"
...
DECLINE_THRESHOLD_CM = 0.0   # diff < 0
RISE_THRESHOLD_CM = 0.0      # diff > 0

# ============================================================
# POINT SETTINGS
# Same style for all points
# ============================================================

# condition 1: class 1 or 2 + decline
POINT_CLASSES_DECLINE = [1, 2]

# condition 2: class 3 + rise
POINT_CLASSES_RISE = [3]

# same grid for all points
POINT_STEP_ROW = 160
POINT_STEP_COL = 160
POINT_START_ROW = 0
POINT_START_COL = 0

# same appearance for all points
POINT_SIZE = 8
POINT_COLOR = "black"
POINT_ALPHA = 0.8
POINT_MARKER = "o"
...
# FACE COLORBAR / LEGEND COLORBAR  --> DO NOT CHANGE
...
# MAP DOMAIN ONLY --> change only the map, not the colorbar
MAP_DIFF_BOUNDARIES = [-50, -30, -10, -5, 0, 5, 10, 30, 50]

# keep same face colors style, but stretch on map to new domain
MAP_DIFF_COLORS = [
    "#a50026",  # < -50
    "#d73027",  # -50 to -30
    "#f46d43",  # -30 to -10
...
def build_systematic_points(condition_mask, transform, step_row, step_col, start_row, start_col):
...
# 0 -> 4
# nodata inside boundary -> 4
# invalid inside boundary -> 4
# ============================================================
...
# ============================================================
# CREATE ONE POINT LAYER
# same color and same marker for all conditions
# ============================================================

point_condition_decline = (
    inside_boundary
    & np.isin(grouped_landcover, POINT_CLASSES_DECLINE)
    & np.isfinite(wtd_diff_cm)
    & (wtd_diff_cm < DECLINE_THRESHOLD_CM)
)

point_condition_rise = (
    inside_boundary
    & np.isin(grouped_landcover, POINT_CLASSES_RISE)
    & np.isfinite(wtd_diff_cm)
    & (wtd_diff_cm > RISE_THRESHOLD_CM)
)

point_condition_all = point_condition_decline | point_condition_rise

xs, ys = build_systematic_points(
    condition_mask=point_condition_all,
    transform=lc_transform,
    step_row=POINT_STEP_ROW,
    step_col=POINT_STEP_COL,
    start_row=POINT_START_ROW,
    start_col=POINT_START_COL
)
...
grouped_handles = [
    ...
    Patch(facecolor=POINT_COLOR, edgecolor="black",
          label="Points: class 1 or 2 + decline, and class 3 + rise"),
]
...
print("\nPoint rule:")
print(f"Decline classes = {POINT_CLASSES_DECLINE}")
print(f"Rise classes = {POINT_CLASSES_RISE}")
print(f"Grid step = every {POINT_STEP_ROW} rows and {POINT_STEP_COL} cols")
print(f"Point color = {POINT_COLOR}")
print(f"Point marker = {POINT_MARKER}")
print(f"Point size = {POINT_SIZE}")

print("\nMap domain only:")
print(MAP_DIFF_BOUNDARIES)

print("\nColorbar kept the same:")
print(CBAR_BOUNDARIES)