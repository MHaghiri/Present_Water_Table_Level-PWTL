...
# units: cm/year
TREND_SLOPE_CM_PER_YEAR = -0.0005
...
# ============================================================
# PNG is raster -> DPI directly controls resolution
PNG_DPI = 1500

# PDF is usually vector -> no true DPI for lines/text.
# BUT dpi DOES control the resolution of any rasterized parts.
PDF_DPI = 1500

# If you want a *true* "high-resolution PDF image", set this True:
# It rasterizes the whole figure into an image and embeds it in a PDF.
RASTERIZE_PDF = False
...
# ============================================================
ORANGE_ARROWS = [
    {"x": datetime(2000, 10, 20), "y": -0.164, "dx_days": 360*1.0, "dy": 0.054},
    {"x": datetime(2001, 11, 1),  "y": -0.112, "dx_days": 365*3,   "dy": 0.000},
    {"x": datetime(2004, 11, 1),  "y": -0.110, "dx_days": 325*1,   "dy": 0.037},
    {"x": datetime(2008, 10, 25), "y": -0.148, "dx_days": 340*1,   "dy": 0.045},
    {"x": datetime(2010, 9, 20),  "y": -0.145, "dx_days": 340*1,   "dy": 0.070},
    {"x": datetime(2014, 10, 20), "y": -0.168, "dx_days": 370*1,   "dy": 0.048},
    {"x": datetime(2016, 9, 25),  "y": -0.134, "dx_days": 370*1,   "dy": 0.028},
    {"x": datetime(2019, 10, 20), "y": -0.120, "dx_days": 371*2.9, "dy": 0.064},
    {"x": datetime(2024, 9, 20),  "y": -0.175, "dx_days": 365*1.0, "dy": 0.039},
]

PURPLE_ARROWS = [
    {"x": datetime(2000, 1, 1),   "y": -0.014, "dx_days": 260*1,    "dy": -0.156},
    {"x": datetime(2005, 9, 25),  "y": -0.076, "dx_days": 365*3.05, "dy": -0.074},
    {"x": datetime(2009, 10, 20), "y": -0.106, "dx_days": 315*1,    "dy": -0.042},
    {"x": datetime(2011, 9, 10),  "y": -0.077, "dx_days": 375*3,    "dy": -0.093},
    {"x": datetime(2015, 11, 1),  "y": -0.120, "dx_days": 320*1,    "dy": -0.015},
    {"x": datetime(2017, 10, 20), "y": -0.108, "dx_days": 365*1.95, "dy": -0.012},
    {"x": datetime(2022, 10, 15), "y": -0.058, "dx_days": 365*1.88, "dy": -0.119},
]
...
def read_panelA_csv(csv_path):
    ...
    return (
        ...
        df["dt"].tolist(),
        df["Total_SLE_cm"].to_numpy(dtype=float),
        df["Groundwater_SLE_cm"].to_numpy(dtype=float),
        df["SurfaceWater_SLE_cm"].to_numpy(dtype=float),
    )
...
def add_trend_line(ax, x_dt, y, label_for_legend=None, force_slope=False):
    ...
    if force_slope:
        slope_cm_per_day = TREND_SLOPE_CM_PER_YEAR / 365.25
        x0 = np.mean(x_num[mask])
        y0 = np.mean(y[mask])
        y_fit = slope_cm_per_day * (x_num - x0) + y0
        slope_used_cm_per_year = TREND_SLOPE_CM_PER_YEAR
    else:
        a, b = np.polyfit(x_num[mask], y[mask], 1)
        y_fit = a * x_num + b
        slope_used_cm_per_year = a * 365.25
    ...
    return line, slope_used_cm_per_year, None

def slope_to_percent_per_year(slope_cm_per_year, y_series, ref_mode="absmean"):
    ...
    return (slope_cm_per_year / denom) * 100.0
...
# SAVE
# ============================================================
# 1) High-res PNG (raster)
plt.savefig(FIG_OUT_PNG, dpi=PNG_DPI, bbox_inches="tight")

# 2) PDF
if not RASTERIZE_PDF:
    # Vector PDF (best for papers). PDF_DPI affects only rasterized parts (if any).
    plt.savefig(FIG_OUT_PDF, format="pdf", bbox_inches="tight", dpi=PDF_DPI)
else:
    # Rasterized PDF: saves a high-res PNG and embeds it into a PDF.
    # This gives a "true DPI-controlled PDF", but it is NOT vector anymore.
    tmp_png = FIG_OUT_PDF.replace(".pdf", f"_raster_{PDF_DPI}dpi.png")
    plt.savefig(tmp_png, dpi=PDF_DPI, bbox_inches="tight")

    # Convert PNG -> PDF using matplotlib (no external tools needed)
...
print("Done.")
print(f"Forced slope value (cm/year) used where forced: {TREND_SLOPE_CM_PER_YEAR}")